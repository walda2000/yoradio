...
void NumWidget::setText(const char* txt) {
  ...
  _getBounds();
  if (strcmp(_oldtext, _text) == 0) return;
  uint16_t realth = _textheight * 8 * 3;
...
void NumWidget::_getBounds() {
  _textwidth = _textWidth(_text) * 3;
}

void NumWidget::_draw() {
#ifndef DSP_LCD
  if(!_active || TIME_SIZE<1 ) return;
  dsp.setTextSize(Clock_GFXfontPtr==nullptr?TIME_SIZE+2:1);
...
bool ClockWidget::_getTime(){
  // pridani :%S sekundy	
  strftime(_timebuffer, sizeof(_timebuffer), "%H:%M:%S", &network.timeinfo);
  return true;
  // Walda mod - aby se sekundy obnovanly co 1 sekundu
